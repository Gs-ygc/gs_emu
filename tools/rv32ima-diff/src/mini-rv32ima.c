...
uint64_t lastTime = 0;
int instrs_per_flip = 1;


void get_regs(void *st) {;} 
...
unsigned long mini_get_ram_size(){
  return ram_amt;
}
...
void si(int n) {
  instrs_per_flip = n;
  lastTime = (fixed_update) ? 0 : (GetTimeMicroseconds() / time_divisor);
  rt=0;
  while (rt<n&& (rt < instct + 1 || instct < 0)) {
    ...
    switch (ret) {
      ...
      case 3:
        ...
        break;
      case 0x7777:
        init_vm();  // syscon code for restart
      case 0x5555:
        printf("POWEROFF@0x%08x%08x\n", core->cycleh, core->cyclel);
        exit(0);  // syscon code for power-off
      default:
...
static void DumpState(struct MiniRV32IMAState *core, uint8_t *ram_image) {
  ...
  printf(
      "Z:%08x ra:%08x sp:%08x gp:%08x tp:%08x t0:%08x t1:%08x t2:%08x s0:%08x "
      "s1:%08x a0:%08x a1:%08x a2:%08x a3:%08x a4:%08x a5:%08x ",
      regs[0], regs[1], regs[2], regs[3], regs[4], regs[5], regs[6], regs[7],
      regs[8], regs[9], regs[10], regs[11], regs[12], regs[13], regs[14],
      regs[15]);
  printf(
      "a6:%08x a7:%08x s2:%08x s3:%08x s4:%08x s5:%08x s6:%08x s7:%08x s8:%08x "
      "s9:%08x s10:%08x s11:%08x t3:%08x t4:%08x t5:%08x t6:%08x\n",
